...
function generateLiveMap() {
    ...
    Papa.parse(f, {header:true, skipEmptyLines:true, complete: function(res) {
        ...
        let stopOpt = window.stopages.map((s, i) => `<option value="${i}">${s.time} | Spd:${s.spd}</option>`).join('');
        ...
        document.getElementById('log').innerText = "Map Ready. Stops: " + window.stopages.length;
    }});
}
